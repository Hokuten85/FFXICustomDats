...
namespace FFXICustomDats
{
    public class PatchItemsFromDB(IConfiguration config, XidbContext context)
    {
        ...
        public static void UpdateItemWeapon(WeaponItem item, ItemWeapon weapon)
        {
            if (weapon != null)
            {
                if (weapon.ItemId == 21568)
                {
                    Console.WriteLine("derp");
                }

                var dbDamage = weapon.Dmg;
                if ((SkillTypeHelpers.SKILLTYPE)weapon.Skill == SkillTypeHelpers.SKILLTYPE.SKILL_HAND_TO_HAND)
                {
                ...
                if (dbDelay > 0 && item.Weapon.DPS != (int)Math.Round(dbDamage * 60.0 / dbDelay * 100))
                {
                ...
                {
                    item.Weapon.SkillType = SkillTypeHelpers.SkillTypeMap.GetValueOrDefault((SkillTypeHelpers.SKILLTYPE)weapon.Skill);
        ...
        public static void UpdateItemEquipment(ArmorItem item, ItemEquipment equipment)
        {
            if (equipment != null)
            {
                ...
                {
                    item.Equipment.Slots = Helpers.DBFlagsToYamlFlags(SlotHelpers.SlotMap, equipment.Slot);
                }

                if (!JobHelpers.IsEqual(item.Equipment.Jobs, equipment.Jobs))
                {
                    item.Equipment.Jobs = JobHelpers.DBFlagsToYamlFlags(equipment.Jobs);
                }
